...
namespace Echo
{
	...
	void ORB::compute(const cv::Mat& image, std::vector<cv::KeyPoint>& keyPoints, std::vector<DescriptorType>& descriptors)
	{
		...
		for (auto& kp : keyPoints)
		{
			...
			for (int i = 0; i < 8; i++) 
			{
				...
				for (int k = 0; k < 32; k++) 
				{
					...
					if (pp.x >= 0 && pp.x < image.cols && pp.y >= 0 && pp.y < image.rows &&
						qq.x >= 0 && qq.x < image.cols && qq.y >= 0 && qq.y < image.rows)
					{
	...
	void ORB::bfMatch(const std::vector<DescriptorType>& desc1, const std::vector<DescriptorType>& desc2, std::vector<cv::DMatch>& matches)
	{
		const int d_max = 40;
